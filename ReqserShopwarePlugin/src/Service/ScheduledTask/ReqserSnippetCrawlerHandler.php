...
class ReqserSnippetCrawlerHandler extends ScheduledTaskHandler
{
    ...
    private array $baseLanguageSnippetSetIDs = [];
            ...
            try {
                ...
                if (isset($customFields['ReqserSnippetCrawl']['active']) && $customFields['ReqserSnippetCrawl']['active'] === true) {
                    ...
                    if (isset($customFields['ReqserSnippetCrawl']['baseLanguage']) && $customFields['ReqserSnippetCrawl']['baseLanguage'] === true) {
                        $this->baseLanguageSnippetSetIDs[] = (string) $row['id'];
                    }
                }
            } catch (\Exception $e) {
    ...
    private function createAllNecessarySnippetTranslations(): void
    {
        ...
        if (count($this->baseLanguageSnippetSetIDs) > 0){
            ...
            if (count($result) > 0) {
                $snippet_set_array = [];
                foreach ($result as $row) {
                    if (in_array($row['snippet_set_id'], $this->baseLanguageSnippetSetIDs)) continue;
                ...
                if (count($snippet_set_array) > 0){
                    ...
                    foreach ($this->baseLanguageSnippetSetIDs as $baseLanguageSnippetSetID) {
                        $result = $this->connection->fetchAllAssociative($sql, [$baseLanguageSnippetSetID]);
                        if (count($result) > 0) {
